...
export default function About() {
  ...
  // --- NEW: Image Reveal Hover Animations ---
  const backImageVariants = {
    hidden: { opacity: 0, x: 20, rotate: 0 },
    // Normal state on scroll
    visible: { 
      opacity: 0.7, 
      x: 0, 
      y: 0, 
    ...
    // Hover state: Slides left, up, brightens, and rotates more
    hover: { 
      opacity: 1, 
      x: "-15%", 
      y: "-5%", 
  ...
  const frontImageVariants = {
    ...
    // Hover state: Slides right, down, and rotates out of the way
    hover: { 
      x: "15%", 
      y: "10%", 
      rotate: 12, 
  ...
  return (
    <section id="about" className="w-full py-24 bg-black flex justify-center px-4 md:px-8">
      ...
      >
        ...
        {/* RIGHT SIDE: Interactive Dual Overlapping Images */}
        {/* Added whileHover="hover" to trigger the swap animation on the children */}
        <motion.div 
          initial="hidden"
          whileInView="visible"
          whileHover="hover"
